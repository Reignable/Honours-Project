...
class ImageProcessor:
    ...
    def _find_ref(self):
        try:
            self._ref_point = self._find_red()[0]
    ...
    def _get_measurement_px(self):
        ...
        max_line_gap = 10
        image_height, image_width = self._edged_image.shape[:2]
        y_limit = self._get_oring_height()
        y_min = image_height
        y_max = 0
        lines = cv2.HoughLinesP(self._edged_image,
                                1,
                                numpy.pi / 180,
                                50,
                                min_line_length,
    ...
    def _convert_px_mm(self, measurement_px):
        pixels_per_mm = self._get_ref_point_width() / self.REF_POINT_KNOWN_WIDTH
        if self._debug:
            utils.debug_print(self.__class__.__name__, '_convert_px_mm', pixels_per_mm)
        return measurement_px / pixels_per_mm

    def get_measurement(self, image_path):
        self._image_path = image_path
        if self._debug:
            utils.debug_print(self.__class__.__name__, 'filepath', self._image_path)
        self._edged_image = self._edge_detect()
        self._find_ref()
        self._find_oring()
        ...
        if measurement_mm <= 0:
            print 'Incorrect measurement produced from image {i}\n'\
                  'Please check settings and try again.'.format(i=self._image_path)
            sys.exit(1)
